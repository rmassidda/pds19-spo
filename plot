...
def createPlot ( name, x, ff, cpp ):
    ...
    plt.plot(x, cpp, '.', color='black', label='C++ Thread')
    plt.xlabel('workers')
...
x = np.array ( x )
createPlot ( "Scalability", x[1:len(x)], fast_flow.scalability()[1:len(x)], cpp_thread.scalability()[1:len(x)] )
